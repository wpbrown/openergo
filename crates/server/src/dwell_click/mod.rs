...
use shared::protocol::server::DwellServerConfig;
...
pub mod driver {
    ...
    use crate::device_events::{ButtonState, Event, EventKind};
    use bachelor::broadcast::spmc::SpmcBroadcastConsumer;
    ...
    impl Driver {
        ...
        pub async fn run(mut self) -> Result<(), Report> {
            ...
            info!("created dwell click virtual device");
        ...
        fn handle_event(&mut self, event: Event) {
            ...
            match event.kind {
                ...
                EventKind::MousePress { state, .. } => {
                    match state {
                        ButtonState::Down => {
                            self.buttons_down = self.buttons_down.saturating_add(1)
                        }
                        ButtonState::Up => self.buttons_down = self.buttons_down.saturating_sub(1),
                    }
                    self.reset_movement();

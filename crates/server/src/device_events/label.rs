...
impl DeviceLabelStore {
    pub fn new() -> Self {
        let mut inner = LabelStore::new();
        let auto_detect = DeviceLabel(inner.get_or_intern_static(AUTO_DETECT_LABEL));
    ...
    pub fn get_or_intern(&mut self, label: &str) -> DeviceLabel {
        DeviceLabel(self.inner.get_or_intern(label))
    }
    ...
    pub fn get(&self, label: &str) -> Option<DeviceLabel> {
        self.inner.get(label).map(DeviceLabel)
    }

...
pub mod driver {
    ...
    async fn run_device(
        ...
        let Some(devnode) = device.devnode() else {
            warn!("device has no devnode, skipping");
            return;
        };
        ...
        info!(
            ...
            name = evdev_device.name().unwrap_or("unknown"),
            physical_path = evdev_device.physical_path().unwrap_or("unknown"),
            "attached",
        );
        ...
        info!(devnode = %devnode.display(), "detached");
    }

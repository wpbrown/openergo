...
use crate::integration::{EndpointCatalog, EndpointLabelStore};
use std::collections::HashMap;
use std::rc::Rc;

/// Build an [`EndpointCatalog`] from the deserialized `devices` map.
...
pub fn init(
    devices: HashMap<String, config::DeviceConfig>,
) -> EndpointCatalog<config::TransportConfigs> {
    let mut labels = EndpointLabelStore::new();
    let mut by_label = litemap::LiteMap::new();

    let mut devices: Vec<(String, config::DeviceConfig)> = devices.into_iter().collect();
    devices.sort_by(|a, b| a.0.cmp(&b.0));
    for (device_key, device) in devices {
        match device {
            config::DeviceConfig::Midi(mut midi) => {
                let mut controls: Vec<(String, config::MidiControlConfig)> =
                ...
                for (label, control) in controls {
                    ...
                    by_label.insert(
                        endpoint,
                        config::TransportConfigs::Midi(config::MidiTransportConfig {
